...
def collect_tweets(mode,query):
	...
	if(mode=='default'): # Full archive tweets. Twitter API V2 Academic access key needed.
		os.system(f'twarc2 search --archive --start-time "{query_time_dt}" --limit 100 "hxxps" raw_output/phishing_{query}.jsonl')
	elif(mode=='lite'): # Regular Twitter API access
		os.system(f'twarc2 search --start-time "{query_time_dt}" --limit 100 "hxxps" raw_output/phishing_{query}.jsonl')
